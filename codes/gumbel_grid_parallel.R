...
loopfun = function(theta){
  ...
  for(i in 1:1000){
    ...
    U = BiCopSim(N, family=4, par=theta)
    u = U[,1]; v = U[,2]
    
    est = BiCopEst(u, v, family=4, method='mle')
    ...
    while(iterating){      
      ...
      dep.list = rep(0,10)
      for(j in 1:10){
        del.loglik = BiCopDeriv(u,v,family=4,par=theta.list[j], deriv="par") / 
          BiCopPDF(u,v,family=4,par=theta.list[j])
